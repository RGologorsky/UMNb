...
class AddAtlas(ItemTransform):
    ...
    def encodes(self, item):
        ...
        item["atlas_image_fn"] = atlas_item["image"]
        item["atlas_label_fn"] = atlas_item["label"]
        
        return item
    
class WhiteoutPituitaryOld(ItemTransform):
    split_idx = None
    ...
    def encodes(self, item):
        ...
        #i = torch.any(torch.any(item[self.label_key].byte(), dim=1), dim=1) # 1 -> 0,2 -> 0 -> 0 left
        #imin, imax = torch.where(i)[0][[0, -1]]
        i = np.any(item[self.label_key].astype(bool), axis=(1, 2))
        imin, imax = np.where(i)[0][[0, -1]]

        # white out
        new_x = item[self.image_key]
        new_x[0:imin] = 0
        new_x[imax:] = 0
        
        # return white-out x
        return {
                self.image_key: new_x, 
                self.label_key: item[self.label_key]
               }

class WhiteoutPituitary10(ItemTransform):
    split_idx = None
...
class WhiteoutPituitaryCoronal(ItemTransform):
    split_idx = None
    ...
    def encodes(self, item):
        ...
        #i = torch.any(torch.any(item[self.label_key].byte(), dim=1), dim=1) # 1 -> 0,2 -> 0 -> 0 left
        #imin, imax = torch.where(i)[0][[0, -1]]
        mask = item[self.label_key].astype(bool)
        ...
        # delta in sag
        new_y_mask1 = np.zeros(mask.shape)
        new_y_mask1[imin:imax] = 1
        
        # delta in cor
        new_y_mask2 = np.zeros(mask.shape)
        new_y_mask2[:,jmin:jmax,:] = 1
        
        new_y_mask = np.logical_or(new_y_mask1, new_y_mask2)
        new_x = item[self.image_key] * new_y_mask
...
def get_train_valid_transforms(pixdim=(1.5,1.5,1.5), full_res=(96, 96, 96), 
                               ...
                               do_whiteout = False, do_coronal = False, do_ten = False):
    ...
    standard_load = [AddAtlas(items), LoadImaged(keys)] if do_condseg else [LoadImaged(keys)]

    if do_whiteout:
        if do_coronal:
            standard_load += [WhiteoutPituitaryCoronal()]
        elif do_ten:
            standard_load += [WhiteoutPituitary10()]
        else:
            print("do whiteout - Unknown type")
    standard_load += [
...
class UndoDictInference(ItemTransform):
...
def get_simple_transforms(pixdim=(1.5,1.5,1.5), full_res=(96, 96, 96), do_condseg=False, items=None, do_whiteout=False):
    ...
    if do_whiteout:
        standard_load += [WhiteoutPituitary()]
        
    standard_load += [
        Spacingd(keys, pixdim=pixdim, mode=["nearest" if "label" in key else "bilinear" for key in keys]),
        NormalizeIntensityd(image_keys, nonzero=True, channel_wise=False),
    ]
    ...
    standard_finish = [
        CenterSpatialCropd(keys=keys, roi_size=full_res),
        ToTensord(keys=keys),
        UndoDictInference(keys=keys),
    ]
    
    return Compose(standard_load + add_ch + all_pad + standard_finish)    

def get_inference_transforms(pixdim=(1.5,1.5,1.5), full_res=(96, 96, 96), do_condseg=False, items=None, do_whiteout=False):

    keys       = ["image", "atlas_image", "atlas_label"] if do_condseg else ["image"]
    image_keys = ["image", "atlas_image"] if do_condseg else ["image"]
    atlas_keys = ["atlas_image", "atlas_label"]
